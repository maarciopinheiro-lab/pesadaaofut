...
export const WhatsAppTab: React.FC<WhatsAppTabProps> = ({ players, selectedDate, matches = [] }) => {
  ...
  const [activeSubTab, setActiveSubTab] = useState<'config' | 'match_config' | 'push_config' | 'render' | 'history' | 'logs'>('config');
  ...
  const loadInitialData = async () => {
    try {
      ...
      // Carregar e sincronizar dados de push notifications
      loadPushData();
  ...
  const loadPushData = async () => {
    ...
    try {
      const [configRes, devicesRes] = await Promise.all([
        fetch('/api/push/config').then((res) => res.json()),
        fetch('/api/push/devices').then((res) => res.json()),
      ]);
      if (configRes && !configRes.error) {
        setPushConfig(configRes);
      }
      if (devicesRes && !devicesRes.error) {
        setPushDevices(devicesRes.devices || []);
      }
    } catch (err) {
      console.warn('Erro ao carregar dados de push:', err);
    } finally {
      setLoadingPush(false);
    }
  };

  const handleSavePushConfig = async (updatedConfig?: any) => {
    setSavingPush(true);
    const bodyToSend = updatedConfig || pushConfig;
    try {
  ...
  const handleSendPushTest = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) {
        showToast(`Envio de teste concluído! Sucessos: ${data.successCount}, Falhas: ${data.failureCount}`, 'success');
        triggerLocalNotification(pushTestTitle || 'Pesadão F.C.', pushTestBody || 'Notificação de teste');
        loadPushData();
      } else {
        showToast(data.error || 'Erro ao enviar notificação de teste.', 'error');
      }
    } catch (err) {
  ...
  return (
    <div className="space-y-8 animation-fade-in pb-24">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              <h2 className="text-xl font-bold">Automação BOT</h2>
      ...
      {/* ========================================================================= */}
      {/* --- SUBTAB: CONFIGURAÇÕES DE NOTIFICAÇÃO (PUSH) --- */}
      {/* ========================================================================= */}
      {activeSubTab === 'push_config' && (
        <div className="space-y-8 animate-fade-in pb-12">
          {/* TOP EXPLANATION AND PERMISSION CARD */}
          <div className="bg-surface-light dark:bg-surface-dark rounded-3xl p-6 md:p-8 border border-gray-200 dark:border-gray-800 shadow-sm flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
            <div className="space-y-2">
              <h3 className="text-lg font-bold flex items-center gap-2">
                <span className="material-icons-outlined text-primary">notifications_active</span>
                Central de Notificações Push (PWA)
              </h3>
              <p className="text-xs text-muted-light max-w-2xl">
                Configure notificações programáveis para disparar alertas diretamente nos celulares e navegadores de todos os membros que ativaram o PWA.
              </p>
            ...
            <button
              ...
              className="w-full md:w-auto px-6 py-3 rounded-2xl text-xs font-black uppercase tracking-wider transition-all flex items-center justify-center gap-2 whitespace-nowrap shadow-lg bg-primary hover:bg-primary-hover text-white shadow-primary/20 hover:scale-[1.02] active:scale-95"
            >
              <span className="material-icons-outlined text-sm">notifications</span>
              {pushPermission === 'granted' ? 'Vincular / Registrar Aparelho' : 'Ativar Notificações'}
            </button>
          </div>

          {/* TWO COLUMN WORKSPACE */}
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            {/* LEFT COLUMN: PROGRAMMABLE NOTIFICATIONS CONFIG */}
            <div className="lg:col-span-8 space-y-6">
              <div className="bg-surface-light dark:bg-surface-dark rounded-3xl p-6 md:p-8 border border-gray-200 dark:border-gray-800 shadow-sm space-y-6">
                <div>
                  <h4 className="text-sm font-black uppercase tracking-wider text-primary">Disparos Agendados</h4>
                  <p className="text-xs text-muted-light mt-1">Configure as duas notificações automáticas e independentes para a partida ou chamados do grupo.</p>
                </div>
                ...
                <div className="space-y-6 divide-y divide-gray-100 dark:divide-gray-800">
                  ...
                  <div className="space-y-4 pt-4 first:pt-0">
                    <div className="flex items-center justify-between">
                      <h5 className="text-xs font-black uppercase tracking-wider flex items-center gap-2 text-gray-700 dark:text-gray-300">
                        <span className="w-5 h-5 rounded-md bg-primary/10 text-primary text-[10px] font-black flex items-center justify-center">1</span>
                        Notificação Programada 1
                      </h5>
                      <label className="relative inline-flex items-center cursor-pointer">
                        <input
                          type="checkbox"
                          checked={pushConfig.notif1_active}
                          onChange={(e) => {
                            const updated = { ...pushConfig, notif1_active: e.target.checked };
                            setPushConfig(updated);
                            handleSavePushConfig(updated);
                          }}
                          className="sr-only peer"
                        />
                        <div className="w-9 h-5 bg-gray-200 peer-focus:outline-none rounded-full peer dark:bg-gray-700 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-4 after:w-4 after:transition-all dark:border-gray-600 peer-checked:bg-green-500"></div>
                        <span className="ml-2 text-[10px] font-bold uppercase text-muted-light">
                          {pushConfig.notif1_active ? 'Ativo' : 'Inativo'}
                        </span>
                      </label>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="space-y-1.5">
                        <label className="text-[10px] font-black uppercase tracking-widest text-muted-light">Título da Notificação</label>
                        <input
                          type="text"
                          value={pushConfig.notif1_title || ''}
                          onChange={(e) => setPushConfig({ ...pushConfig, notif1_title: e.target.value })}
                          placeholder="Ex: Confirmação de Presença"
                          className="w-full px-4 py-2.5 rounded-xl text-xs font-bold bg-gray-50 dark:bg-black/20 border border-gray-200 dark:border-gray-800 focus:outline-none focus:border-primary"
                        />
                      </div>

                      <div className="grid grid-cols-2 gap-2">
                        <div className="space-y-1.5">
                          <label className="text-[10px] font-black uppercase tracking-widest text-muted-light">Data de Envio</label>
                          <input
                            type="date"
                            value={pushConfig.notif1_date || ''}
                            onChange={(e) => setPushConfig({ ...pushConfig, notif1_date: e.target.value })}
                            className="w-full px-4 py-2.5 rounded-xl text-xs font-bold bg-gray-50 dark:bg-black/20 border border-gray-200 dark:border-gray-800 focus:outline-none focus:border-primary"
                          />
                        </div>
                        <div className="space-y-1.5">
                          <label className="text-[10px] font-black uppercase tracking-widest text-muted-light">Horário</label>
                          <input
                            type="time"
                            value={pushConfig.notif1_time || '09:00'}
                            onChange={(e) => setPushConfig({ ...pushConfig, notif1_time: e.target.value })}
                            className="w-full px-4 py-2.5 rounded-xl text-xs font-bold bg-gray-50 dark:bg-black/20 border border-gray-200 dark:border-gray-800 focus:outline-none focus:border-primary"
                          />
                        </div>
                      </div>
                    ...
                    <div className="space-y-1.5">
                      <label className="text-[10px] font-black uppercase tracking-widest text-muted-light">Mensagem (Corpo)</label>
                      <textarea
                        rows={2}
                        value={pushConfig.notif1_body || ''}
                        onChange={(e) => setPushConfig({ ...pushConfig, notif1_body: e.target.value })}
                        placeholder="Ex: Já confirmou sua presença no rachão de domingo? Acesse o app e vote!"
                        className="w-full px-4 py-2.5 rounded-xl text-xs font-semibold bg-gray-50 dark:bg-black/20 border border-gray-200 dark:border-gray-800 focus:outline-none focus:border-primary resize-none"
                      />
                    </div>

                    <div className="flex items-center justify-between text-[11px] bg-gray-50 dark:bg-black/10 p-3 rounded-2xl">
                      <span className="text-muted-light font-medium flex items-center gap-1">
                        <span className="material-icons-outlined text-xs">info</span>
                        Status do último envio:
                      </span>
                      <span className={`font-black uppercase text-[10px] tracking-wider ${
                        pushConfig.notif1_status === 'sent'
                          ? 'text-green-500'
                          : pushConfig.notif1_status === 'failed'
                          ? 'text-red-500'
                          : 'text-yellow-500'
                      }`}>
                        {pushConfig.notif1_status === 'sent' ? '🟢 Enviado' : pushConfig.notif1_status === 'failed' ? '🔴 Falhou' : '⏳ Aguardando'}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="space-y-4 pt-6">
                    <div className="flex items-center justify-between">
                      <h5 className="text-xs font-black uppercase tracking-wider flex items-center gap-2 text-gray-700 dark:text-gray-300">
                        <span className="w-5 h-5 rounded-md bg-primary/10 text-primary text-[10px] font-black flex items-center justify-center">2</span>
                        Notificação Programada 2
                      </h5>
                      <label className="relative inline-flex items-center cursor-pointer">
                        <input
                          type="checkbox"
                          checked={pushConfig.notif2_active}
                          onChange={(e) => {
                            const updated = { ...pushConfig, notif2_active: e.target.checked };
                            setPushConfig(updated);
                            handleSavePushConfig(updated);
                          }}
                          className="sr-only peer"
                        ...
                        <div className="w-9 h-5 bg-gray-200 peer-focus:outline-none rounded-full peer dark:bg-gray-700 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-4 after:w-4 after:transition-all dark:border-gray-600 peer-checked:bg-green-500"></div>
                        <span className="ml-2 text-[10px] font-bold uppercase text-muted-light">
                          {pushConfig.notif2_active ? 'Ativo' : 'Inativo'}
                        </span>
                      </label>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="space-y-1.5">
                        ...
                        <input
                          type="text"
                          value={pushConfig.notif2_title || ''}
                          onChange={(e) => setPushConfig({ ...pushConfig, notif2_title: e.target.value })}
                          placeholder="Ex: Últimas Vagas do Jogo"
                          className="w-full px-4 py-2.5 rounded-xl text-xs font-bold bg-gray-50 dark:bg-black/20 border border-gray-200 dark:border-gray-800 focus:outline-none focus:border-primary"
                        />
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <div className="space-y-1.5">
                          ...
                          <input
                            type="date"
                            value={pushConfig.notif2_date || ''}
                            onChange={(e) => setPushConfig({ ...pushConfig, notif2_date: e.target.value })}
                            className="w-full px-4 py-2.5 rounded-xl text-xs font-bold bg-gray-50 dark:bg-black/20 border border-gray-200 dark:border-gray-800 focus:outline-none focus:border-primary"
                          />
                        </div>
                        <div className="space-y-1.5">
                          ...
                          <input
                            type="time"
                            value={pushConfig.notif2_time || '09:00'}
                            onChange={(e) => setPushConfig({ ...pushConfig, notif2_time: e.target.value })}
                            className="w-full px-4 py-2.5 rounded-xl text-xs font-bold bg-gray-50 dark:bg-black/20 border border-gray-200 dark:border-gray-800 focus:outline-none focus:border-primary"
                          />
                        </div>
                      </div>
                    ...
                    <div className="space-y-1.5">
                      ...
                      <textarea
                        rows={2}
                        value={pushConfig.notif2_body || ''}
                        onChange={(e) => setPushConfig({ ...pushConfig, notif2_body: e.target.value })}
                        placeholder="Ex: Última chance para confirmar presença na partida! O sorteio dos times ocorre logo mais."
                        className="w-full px-4 py-2.5 rounded-xl text-xs font-semibold bg-gray-50 dark:bg-black/20 border border-gray-200 dark:border-gray-800 focus:outline-none focus:border-primary resize-none"
                      />
                    </div>
                    ...
                    <div className="flex items-center justify-between text-[11px] bg-gray-50 dark:bg-black/10 p-3 rounded-2xl">
                      ...
                      <span className={`font-black uppercase text-[10px] tracking-wider ${
                        pushConfig.notif2_status === 'sent'
                          ? 'text-green-500'
                          : pushConfig.notif2_status === 'failed'
                          ? 'text-red-500'
                          : 'text-yellow-500'
                      }`}>
                        {pushConfig.notif2_status === 'sent' ? '🟢 Enviado' : pushConfig.notif2_status === 'failed' ? '🔴 Falhou' : '⏳ Aguardando'}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex justify-end pt-4">
                  <button
                    ...
                    disabled={savingPush}
                    className="w-full sm:w-auto px-6 py-3 rounded-2xl text-xs font-black uppercase tracking-wider bg-primary hover:bg-primary-hover text-white shadow-lg shadow-primary/20 hover:scale-[1.02] active:scale-95 transition-all disabled:opacity-50"
                  >
                    {savingPush ? 'Salvando...' : 'Salvar Configurações'}
                  </button>
                </div>
              </div>
            </div>

            {/* RIGHT COLUMN: DIRECT MANUAL TEST & ACTIVE DEVICES LIST */}
            <div className="lg:col-span-4 space-y-6">
              {/* TEST TRIGGER CARD */}
              <div className="bg-surface-light dark:bg-surface-dark rounded-3xl p-6 border border-gray-200 dark:border-gray-800 shadow-sm space-y-4">
                <div>
                  <h4 className="text-xs font-black uppercase tracking-wider text-primary">Disparar Teste Manual</h4>
                  <p className="text-[11px] text-muted-light mt-0.5">Envia um alerta instantâneo para todos os dispositivos inscritos.</p>
                </div>

                <div className="space-y-3">
                  <div className="space-y-1">
                    <label className="text-[9px] font-black uppercase tracking-widest text-muted-light">Título do Teste</label>
                    <input
                      type="text"
                      value={pushTestTitle}
                      onChange={(e) => setPushTestTitle(e.target.value)}
                      placeholder="Ex: Gol do Pesadão!"
                      className="w-full px-3 py-2 rounded-xl text-xs font-bold bg-gray-50 dark:bg-black/20 border border-gray-200 dark:border-gray-800 focus:outline-none focus:border-primary"
                    />
                  </div>

                  <div className="space-y-1">
                    <label className="text-[9px] font-black uppercase tracking-widest text-muted-light">Mensagem do Teste</label>
                    <textarea
                      rows={2}
                      value={pushTestBody}
                      onChange={(e) => setPushTestBody(e.target.value)}
                      placeholder="Escreva a mensagem aqui..."
                      className="w-full px-3 py-2 rounded-xl text-xs font-semibold bg-gray-50 dark:bg-black/20 border border-gray-200 dark:border-gray-800 focus:outline-none focus:border-primary resize-none"
                    />
                  </div>

                  <button
                    onClick={handleSendPushTest}
                    disabled={sendingPushTest || pushDevices.length === 0}
                    className="w-full py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest bg-yellow-500 hover:bg-yellow-600 text-black shadow-lg shadow-yellow-500/10 active:scale-95 transition-all disabled:opacity-50 disabled:scale-100 disabled:cursor-not-allowed"
                  >
                  ...
                  {pushDevices.length === 0 && (
                    <p className="text-[10px] text-yellow-600 dark:text-yellow-400 font-semibold text-center mt-1">
                      ⚠️ Nenhum dispositivo registrado para receber alertas.
                    </p>
                  )}
                </div>
              </div>
              ...
              <div className="bg-surface-light dark:bg-surface-dark rounded-3xl p-6 border border-gray-200 dark:border-gray-800 shadow-sm space-y-4">
                ...
                <div className="space-y-2.5 max-h-80 overflow-y-auto pr-1">
                  {loadingPush ? (
                    ...
                    <div className="py-8 text-center text-xs text-muted-light opacity-60">
                      Nenhum dispositivo cadastrado ainda. Conecte pelo botão acima!
                    </div>
                  ) : (
                    pushDevices.map((dev) => {
                      let parsedInfo: any = {};
