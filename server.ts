...
async function startServer() {
  ...
  // --- API ROTAS PUSH NOTIFICATIONS (FCM) ---
  ...
  app.post('/api/push/send-test', async (req, res) => {
    try {
      ...
      if (!title || !body) {
        res.status(400).json({ error: 'Título e mensagem são obrigatórios para o teste.' });
        return;
      }
      ...
      console.error('[API Push] Erro no envio de teste:', err);
      res.status(500).json({ error: err.message });
    }
  });

  // Listar todos os aparelhos registrados (Painel Admin)
  ...
  cron.schedule('* * * * *', async () => {
    try {
      ...
      console.error('[CronPush] Erro no ciclo de agendamentos push:', err);
    }
  });

  // Service Workers com headers estritos de JS e PWA
